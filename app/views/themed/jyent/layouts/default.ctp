	...
	<div id="container" class="container">
		...
		<ul id="navbar">
			<li style="position:relative;top:14px;border:none;background:transparent;padding:0"><?php echo $html->image('leftcurve.png'); ?></li>
                        ...
                        <li><?php echo $html->link('CONTACT US', '/contacts/add'); ?></li>
                        <li><?php echo $html->link('SITEMAP', '/sitemaps'); ?></li>
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		var category = "<?php echo $this->params['pass'][0]; ?>";
		var producttype = "<?php echo $session->read('Product.type');?>";
		if(controller == 'products') {
			if(action == 'index' || action == 'admin_index') {
				$('#sidenav').load('/products/sidenav/' + category);
			}
			if(action == 'view') {
				$('#sidenav').load('/products/sidenav/' + producttype);
			}
		} else {
			if(controller != 'nodes') {
				$('#sidenav').load('/products/sidenav/'+producttype);
