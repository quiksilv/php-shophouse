...
?>
	<tr<?php echo $class;?>>
		<!--td>
			<span class="rn_editable" title="product<?php echo $product['Product']['id']; ?>"><?php echo $product['Product']['reference_number']; ?></span>
		...
		<td>
			<span class="name_editable" title="product<?php echo $product['Product']['id']; ?>"><?php echo $product['Product']['name']; ?></span>
			<span class="error" style="display:none"></span>
			<div class="desc_editable" title="product<?php echo $product['Product']['id']; ?>"><?php echo $text->truncate($product['Product']['description'], 50); ?></div>
			<!-- handle tags -->
			<div class="tags">
				<?php foreach($product['Tag'] as $tags): ?>
					<span class="tags_editable" title="product<?php echo $product['Product']['id']; ?>"><?php echo $tags['tag']; ?></span>
				<?php endforeach; ?>
			</div>
			<span class="error" style="display:none"></span>
		</td>
		<td>
			<span class="selecteditable" title="product<?php echo $product['Product']['id']; ?>"><?php echo $product['Product']['getpath']; ?></span>
			<span class="error" style="display:none"></span>
		</td>
		<td>
...
$(document).ready(function() {
	...
	/** use the proper dataType parameter please **/
	$.getJSON('/admin/products/nodes', function(data) {
		$(".selecteditable").editable({
			...
			onSubmit:function(content) {
				...
				$.post('/admin/products/update/' + $(this).attr("title").substring(7),
					{'data[Product][parent_id]': content.current.value},
					function(data) {
						/** $(this) in this jquery function will point to the function itself, 
						so we assign the HTML span element in the variable current **/
						current.append(content.current.text);
					},
					'json'
				);			
			}	
